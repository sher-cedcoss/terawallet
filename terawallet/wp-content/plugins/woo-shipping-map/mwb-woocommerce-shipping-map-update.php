...
if( ! class_exists( 'Mwb_Woocommerce_Shipping_Map_Update' ) ) 
{
	class Mwb_Woocommerce_Shipping_Map_Update 
	{
		public function __construct() {
			register_activation_hook(MWB_WOOCOMMERCE_SHIPPING_MAP_BASE_FILE, array($this, 'mwb_check_activation'));
			add_action('mwb_check_event', array($this, 'mwb_check_update'));
			add_filter( 'http_request_args', array($this, 'mwb_updates_exclude'), 5, 2 );
			register_deactivation_hook(MWB_WOOCOMMERCE_SHIPPING_MAP_BASE_FILE, array($this, 'mwb_check_deactivation'));
		}	
		...
		public function mwb_check_update() 
		{
			global $wp_version;
			global $update_check;
			$plugin_folder = plugin_basename( dirname( MWB_WOOCOMMERCE_SHIPPING_MAP_BASE_FILE ) );
			$plugin_file = basename( ( MWB_WOOCOMMERCE_SHIPPING_MAP_BASE_FILE ) );
			...
			$postdata = array(
				'action' => 'check_update',
				'license_key' => MWB_WOOCOMMERCE_SHIPPING_MAP_LICENSE_KEY
			);
		...
		public function mwb_plugin_get($i) 
		{
			...
			$plugin_folder = get_plugins( '/' . plugin_basename( dirname( MWB_WOOCOMMERCE_SHIPPING_MAP_BASE_FILE ) ) );
			$plugin_file = basename( ( MWB_WOOCOMMERCE_SHIPPING_MAP_BASE_FILE ) );
			return $plugin_folder[$plugin_file][$i];
		}
	}
	new Mwb_Woocommerce_Shipping_Map_Update();
}		
?>